...
if [ -f "$WALLPAPER_DIR/$SELECTED_WALLPAPER.jpg" ]; then
    ...
    sed -i -e "s|preload = .*|preload = $HOME/Wallpapers/$SELECTED_WALLPAPER.png|" \
           -e "s|wallpaper = ,.*|wallpaper = ,$HOME/Wallpapers/$SELECTED_WALLPAPER.png|" "$TARGET_FILE"
    sed -i -e "s|path = .*|path = $HOME/Wallpapers/$SELECTED_WALLPAPER.png|" "$TARGET_FILE2"
